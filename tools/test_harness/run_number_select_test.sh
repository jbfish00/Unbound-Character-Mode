#!/bin/bash
# Live catch-gate test driver (see number_select_test.gdb). Phase-1 masher
...
LOG="$ROOT/build/number_select_test.log"
...
timeout 700 gdb-multiarch -batch -x "$HERE/number_select_test.gdb" "$ELF" >"$LOG" 2>&1
