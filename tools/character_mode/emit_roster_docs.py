...
def load_unselectable():
    ...
    NOT used to filter these docs, deliberately. The docs must describe what the
    menu really offers, and this game's selection path is not gated yet -- that
    is separate engine work. Until it lands every character is selectable, so
    every character is documented, and the header reports which ones are thin.
    Filtering here first would advertise a restriction the ROM does not enforce.
    """
