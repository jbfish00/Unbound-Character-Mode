...
TRADES = (2543500, 9, 12, ((0, 470), (1, 633), (2, 237), (3, 101), (4, 973), (5, 644), (6, 661), (7, 114), (8, 86)))
# (rom address of a givemon, species, level) -- only the sites that matter to a
# gate, each hand-decoded.
GIFTS = ((136762999, 43, 20), (166131637, 1086, 35), (166259749, 1100, 50))
# gate site -> ((how its species can arrive without a catch, what it costs), ...)
REACHABLE = {
 0x09e63112: (
  ('Pikachu 1086, same NPC, first site',
   'REAL COST -- same Air Balloon'),
 ),
 0x09e63191: (
  ('Pikachu 1086 via the 0x09E6F7B5 gift (level 35)',
   'REAL COST -- the Air Balloon has no mart entry and no other scripted'
    ' source'),
 ),
 0x09e68c86: (
  ('Oddish 43 via the 0x0826D677 pick-one-of-three gift',
   'NOT reachable in practice -- the scientist consumes THIRTY Oddish'
    ' and the gift grants one'),
 ),
 0x09e8eaed: (
  ('Pichu 1100 via the 0x09E8EC25 gift (level 50)',
   'REAL COST -- the Destiny Knot has no mart entry and no other'
    ' scripted source'),
 ),
 0x09e8eb95: (
  ('Pichu 1100 via the same gift',
   'SPECIES_LOCKED anyway -- the reward is another Pichu'),
 ),
}
