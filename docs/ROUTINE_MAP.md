...
- **Catch message ("Gotcha!") — table fully bounded, real code now inspected via Ghidra (upgraded twice this session).** File offset `0x3FD7A2`'s pointer is referenced at `0x3FE338`, inside a table now fully bounded by scanning outward for the full run of valid `0x08xxxxxx`-range pointers: starts at `0x3FDF3C`, ends (exclusive) at `0x3FE514` — **374 entries**, a size right in line with pret's known `gBattleStringsTable`. Searching for pointer references to the table's *base* (`0x3FDF3C`) found **26 hits**, densely clustered in file offsets `0xCEFEC`–`0xD8494` (~57 KB), plus 3 outliers.
  ...
  - With Thumb mode forced, 4 of the 26 sites (`0x080CF068`, `0x080CF328`, `0x080CF3F0`, `0x080CFE50`) show real, sensible code, and 2 more (`0x080CFB28`, `0x080D8494`) turned out to be thunks (`b <target>`) pointing elsewhere, not inspected further yet. **Important correction to the earlier "single dispatcher" assumption**: the real code shows a *pattern*, not one shared function — each site does something like `cmp rX,#0x4 / bne <else> / ldr r0,[table_base] / ... / lsl r1,r1,#0x3 (×8 stride) / add r1,r1,r0 / ...` vs. an else-branch computing `r1 = rX + 0xD` and calling a different function. The `×8` stride means at least some of these accesses treat the table (or a sub-region of it) as **8-byte records, not 4-byte pointers** — this doesn't necessarily contradict the "gBattleStringsTable" identification (different call sites can index the same base differently for different needs), but it means the table's exact semantic shape is **not fully resolved yet**, and the "26 call sites = 26 generic battle-message print sites" framing from the previous pass was an oversimplification. Still strong evidence Unbound's internals are vanilla/CFRU-shaped (real, coherent ARM/Thumb code with sensible control flow, not something exotic) — risk #1 remains substantially de-risked — but the next step should be running Ghidra's **Decompiler** (not just disassembly) on these 4 functions for a clearer high-level view before assuming which one, if any, is the actual catch-success handler.
